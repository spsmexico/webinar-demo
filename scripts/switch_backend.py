...
siguiente orden:
1.- Desactiva el trail "webinar-cloudtrail" en la región primaria y lo habilita en la secundaria.
2.- Desactiva todas las reglas de EventBridge que inician con "webinar-" en la región primaria y las habilita en la secundaria.
...
de apuntar a la rama dr/{ambiente}/{region}-apagado y mostrar WEBINAR.
...
def intercambiar_trail():
    ...
    try:
        ...
        trail_primario_arn = jmespath.search("Trails[?starts_with(Name,'webinar-cloudtrail')].TrailARN", trail_primario_arn)
    ...
    trail_secundario_arn = jmespath.search("Trails[?starts_with(Name,'webinar-cloudtrail')].TrailARN", trail_secundario_arn)
...
def intercambiar_reglas_eventbridge():
    ...
    try:
        ...
        nombres_reglas = jmespath.search("Rules[?starts_with(Name,'webinar-')].Name", reglas)
    except:
    ...
    nombres_reglas_encender = jmespath.search("Rules[?starts_with(Name,'webinar-')].Name", reglas_encender)
...
def intercambiar_apigateway():
    ...
    try:
        ...
        api_id = jmespath.search("Items[?Name == 'webinar-apigateway'].ApiId", response)
    ...
    pprint.pprint(response)
    api_id_secundario = jmespath.search("Items[?Name == 'webinar-apigateway'].ApiId", response)
    ...
    try:    
        audience = obtener_parametro(ssm_secundario, 'webinar-user-pool-client-id-frontend', 'list')
...
def intercambiar_appsync():
    ...
    try:
        ...
        api_primario = jmespath.search("graphqlApis[?starts_with(name,'webinar-frontend-appsync-api-')]", appsync_apis)
    ...
    try:
        ...
        api_secundario = jmespath.search("graphqlApis[?starts_with(name,'webinar-frontend-appsync-api-')]", appsync_apis_secundario)
    ...
    try:
        user_pool_id = obtener_parametro(ssm_secundario, "webinar-user-pool-id", "string")
